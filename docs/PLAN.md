...
- **WAR (wins above replacement) — adapted for MA state legislative races.**
  ...
  **We are explicitly adapting, not replicating, Split Ticket's method** —
  ...
  - *Different baseline inputs*: **v1** baseline = district partisan lean
    only (§ above). **v2** (built — see `generate_site_data.fit_incumbency_effect`/
    `apply_war_v2`) adds one fundamental on top of v1: incumbency, fit as
    the gap between incumbents' and non-incumbents' mean WAR v1 over every
    contested major-party race in the full 2002-2024 backfill (+0.1245,
    n=413 incumbent / 1081 non-incumbent as of the last full pipeline run —
    see `site/_data/war_v2.yml` for the live figure and the methodology
    page for the full writeup). Campaign finance was also planned as a v2
    fundamental, same as Split Ticket's model, but this project's own
    weighting, not theirs — deferred to a future **v3**: the OCPF data
    fetched so far only covers year 2022, not enough years to fit an
    honest term across the full backfill. Both WAR v1 and v2 are shown
